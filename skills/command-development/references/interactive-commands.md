...
Comprehensive guide to creating commands that gather user feedback and make decisions through the question tool.
...
For these cases, use the **question tool** within command execution rather than relying on command arguments.
...
This command will guide you through configuring the project with a series of questions.
...
Based on the answers received from the question tool:
...
Create a project configuration file with:
...
Use the question tool to ask about deployment basics.
...
```markdown
Use the question tool for enabling features:
...
## Command Patterns with the question Tool
...
Use the question tool with multiple questions in one call:
...
Use the question tool for member details:
...
Use the question tool with multiSelect:
...
After calling the question tool, verify answers received:
...
Use the question tool to collect settings.
...
Use the question tool for core settings.

Save to `.opencode/config-partial.yml`
...
Use the question tool for settings based on Phase 1 answers.
...
Use the question tool for options that require explanation.
```
...
Now use the question tool to choose:
...
With the question tool, commands become interactive wizards that guide users through complex decisions while maintaining the clarity that simple arguments provide for straightforward inputs.
