...
head -n 20 .opencode/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .opencode/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .opencode/commands/*.md

# Check file is in correct location
test -f .opencode/commands/my-command.md && echo "Found" || echo "Missing"
```
...
if echo "$FRONTMATTER" | grep -q "^model:"; then
  ...
  if ! echo "anthropic/claude-sonnet-4 anthropic/claude-opus-4-5 anthropic/claude-haiku-4-5" | grep -qw "$MODEL"; then
    echo "ERROR: Invalid model '$MODEL' (must be anthropic/claude-sonnet-4, anthropic/claude-opus-4-5, or anthropic/claude-haiku-4-5)"
    exit 1
  fi
...
# Check 'subtask' field if present
if echo "$FRONTMATTER" | grep -q "^subtask:"; then
  SUBTASK=$(echo "$FRONTMATTER" | grep "^subtask:" | cut -d: -f2 | tr -d ' ')
  if ! echo "$SUBTASK" | grep -qE "^(true|false)$"; then
    echo "ERROR: Invalid subtask value '$SUBTASK' (must be true or false)"
    exit 1
  fi
  echo "✓ Subtask field valid: $SUBTASK"
fi
...
cat > .opencode/commands/test-bash.md << 'EOF'
---
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.opencode/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .opencode/commands/; then
...
```bash
# Monitor Opencode during command execution
# In terminal 1:
opencode
...
```bash
# Check file location
ls -la .opencode/commands/my-command.md

# Check permissions
chmod 644 .opencode/commands/my-command.md

# Check syntax
head -n 20 .opencode/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .opencode/commands/my-command.md
grep '\$ARGUMENTS' .opencode/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .opencode/commands/test-args.md
```
...
grep '!\`' .opencode/commands/my-command.md
